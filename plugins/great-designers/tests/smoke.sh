#!/usr/bin/env bash
# Plugin smoke tests for great-designers. Catches the regressions that matter —
# frontmatter validity, persona-count alignment between agents/ and DXT bundle,
# version coherence, DXT tool/handler matching, Ive-redirect presence.
...
# v0.1 ships exactly 9 personas. Jony Ive lives in great-minds; not duplicated here.
...
for skill_dir in skills/*/; do
  ...
  # Convert kebab-case to snake_case for DXT tool name (designers-channel -> designers_channel)
...
# ---------- 5. Jony Ive cross-dispatch redirect ----------

echo "Checking Jony Ive redirect is present..."
if ! grep -q "great-minds:jony-ive-designer" distribution/dxt/server/index.js; then
  red "  FAIL: DXT server is missing the Jony Ive -> great-minds redirect"
  ERRORS=$((ERRORS + 1))
fi
if ls agents/*ive* 2>/dev/null | grep -q .; then
  red "  FAIL: Jony Ive persona file found in agents/ — he lives in great-minds, not here"
  ERRORS=$((ERRORS + 1))
fi
