#!/usr/bin/env bash
# Plugin smoke tests for great-engineers. Catches the regressions that matter —
# frontmatter validity, persona-count alignment between agents/ and DXT bundle,
# version coherence, DXT tool/handler matching, Margaret-redirect presence.
...
# v0.1 ships exactly 9 personas. Margaret Hamilton lives in great-minds; not duplicated here.
...
for skill_dir in skills/*/; do
  ...
  # Convert kebab-case to snake_case for DXT tool name (engineers-channel -> engineers_channel)
...
# ---------- 5. Margaret Hamilton cross-dispatch redirect ----------

echo "Checking Margaret Hamilton redirect is present..."
if ! grep -q "great-minds:margaret-hamilton-qa" distribution/dxt/server/index.js; then
  red "  FAIL: DXT server is missing the Margaret Hamilton -> great-minds redirect"
  ERRORS=$((ERRORS + 1))
fi
if ls agents/*margaret* 2>/dev/null | grep -q .; then
  red "  FAIL: Margaret Hamilton persona file found in agents/ — she lives in great-minds, not here"
  ERRORS=$((ERRORS + 1))
fi
