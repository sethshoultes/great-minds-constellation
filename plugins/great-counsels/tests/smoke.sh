#!/usr/bin/env bash
# Plugin smoke tests for great-counsels. Catches the regressions that matter —
# frontmatter validity, persona-count alignment between agents/ and DXT bundle,
# version coherence, DXT tool/handler matching, Aurelius-redirect presence,
# disclaimer presence in critical files.
...
# v0.1 ships exactly 9 personas. Marcus Aurelius lives in great-minds; not duplicated here.
...
for skill_dir in skills/*/; do
  ...
  # Convert kebab-case to snake_case for DXT tool name (counsels-channel -> counsels_channel)
...
# ---------- 5. Marcus Aurelius cross-dispatch redirect ----------

echo "Checking Marcus Aurelius redirect is present..."
if ! grep -q "great-minds:marcus-aurelius-mod" distribution/dxt/server/index.js; then
  red "  FAIL: DXT server is missing the Marcus Aurelius -> great-minds redirect"
  ERRORS=$((ERRORS + 1))
fi
if ls agents/*aurelius* 2>/dev/null | grep -q .; then
  red "  FAIL: Marcus Aurelius persona file found in agents/ — he lives in great-minds, not here"
  ERRORS=$((ERRORS + 1))
fi

# ---------- 6. Not-legal-advice disclaimer presence ----------

echo "Checking NOT LEGAL ADVICE disclaimer is present in critical files..."
for f in README.md MANUAL.md ORCHESTRATING.md distribution/dxt/README.md distribution/dxt/manifest.json; do
  if [ ! -f "$f" ]; then
    red "  FAIL: $f does not exist"
    ERRORS=$((ERRORS + 1))
    continue
  fi
  if ! grep -qi "not legal advice\|NOT LEGAL ADVICE" "$f"; then
    red "  FAIL: $f does not contain the not-legal-advice disclaimer"
    ERRORS=$((ERRORS + 1))
  fi
done

# Also check the DXT server enforces the disclaimer in tool prompts.
if ! grep -qi "not legal advice\|NOT LEGAL ADVICE" distribution/dxt/server/index.js; then
  red "  FAIL: DXT server does not contain the not-legal-advice disclaimer"
  ERRORS=$((ERRORS + 1))
fi
